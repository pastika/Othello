...
void OthelloPlayerCES::returnPlay(const OthelloBoard<8, 8>& board, int& x, int& y)
{
    ...
    for(auto& move : board.getValidPlays())
    {
        ...
        {
            plays[2 + std::min(int(fabs(3.5 - move.first)), int(fabs(3.5 - move.second)))].push_back(move);
        }
        else
        {
            plays[5 + 4*std::max(int(fabs(3.5 - move.first)), int(fabs(3.5 - move.second))) + std::min(int(fabs(3.5 - move.first)), int(fabs(3.5 - move.second)))].push_back(move);
        }
    }
